...
export const Navbar: React.FC<NavbarProps> = ({ onOpenChat, theme = 'dark', onToggleTheme }) => {
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      const sections = ['hero', 'about', 'skills', 'projects', 'experience', 'qualifications', 'activities', 'contact'];
    ...
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex items-center justify-between">
        ...
        <a href="#hero" className="flex items-center gap-2.5 group shrink-0">
          <div className="w-10 h-10 rounded-full bg-[#88EC11] p-[2px] shadow-lg shadow-[#88EC11]/30 group-hover:scale-105 group-hover:shadow-[#88EC11]/60 transition-all overflow-hidden shrink-0">
            <img
              src="/profile.jpg"
          ...
          <div className="flex flex-col min-w-0">
            <span className="text-sm lg:text-base font-bold tracking-tight text-white group-hover:text-[#88EC11] transition-colors flex items-center gap-1.5 truncate">
              <span>Kaveesha Vimukthi</span>
              <span className="w-2 h-2 rounded-full bg-[#88EC11] animate-pulse shrink-0" />
            </span>
            <span className="hidden xl:block text-[9px] font-mono-code text-[#88EC11] tracking-wider uppercase font-semibold truncate">
        ...
        {/* Desktop Nav Links */}
        <nav className="hidden md:flex items-center gap-0.5 lg:gap-1 bg-[#18181c]/90 p-1 lg:p-1.5 rounded-full border border-white/10 backdrop-blur-md shadow-inner">
          {navLinks.map((link) => {
            ...
            return (
              <a
                key={link.name}
                href={link.href}
                className={`relative px-2.5 lg:px-3.5 py-1.5 text-xs lg:text-sm font-semibold rounded-full transition-all duration-200 ${
        ...
        <div className="hidden md:flex items-center gap-2.5 shrink-0">
          {onToggleTheme && (
            <button
              ...
              className="p-2 rounded-full bg-white/5 hover:bg-white/10 text-slate-300 hover:text-[#88EC11] border border-white/10 transition-colors"
          ...
          <button
            onClick={onOpenChat}
            className="px-3.5 py-1.5 lg:px-4 lg:py-2 rounded-full bg-[#88EC11]/10 hover:bg-[#88EC11]/20 text-[#88EC11] border border-[#88EC11]/40 text-xs font-mono-code font-bold flex items-center gap-2 transition-all hover:scale-105 shadow-md shadow-[#88EC11]/10"
          >
